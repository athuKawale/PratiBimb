...
def virtualcamera(globals, swap_model, streamobs, use_xseg, use_mouthrestore, cam_num,width,height):
    ...
    subsample_size = globals.subsample_size


    options = ProcessOptions(swap_model, get_processing_plugins("mask_xseg" if use_xseg else None), globals.distance_threshold, globals.blend_ratio,
    ...
    while cam_active:
        ...
        if len(globals.INPUT_FACESETS) > 0:
